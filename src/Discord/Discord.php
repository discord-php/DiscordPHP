        ...
        if ($message->isBinary()) {
            if ($this->zlibDecompressor) {
                ...
                if (($inflated = inflate_add($this->zlibDecompressor, $this->payloadBuffer)) !== false) {
                    ...
                    $this->logger->error('failed to inflate payload buffer', ['payload' => $this->payload, 'buffer' => $this->payloadBuffer, 'payload hex' => bin2hex($this->payload), 'buffer hex' => bin2hex($this->payloadBuffer)]);
                }
                $this->payloadBuffer = '';
            } else {
